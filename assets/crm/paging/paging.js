...
(function(root, factory) {
	...
	Paging.prototype = {
		init: function(settings) {
			...
			this.target = $(this.settings.target);if (this.target.children('.ui-paging-container').length > 0) {
				this.container = this.target.children('.ui-paging-container').eq(0);
			} else {
		...
		format: function() {
			...
			html += '<li class="ui-paging-ellipse" class="ui-pager">共 ' + this.settings.count + ' 条记录</li>';
			html += '</ul>';
