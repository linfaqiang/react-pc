...
let fileTypes = new Set(['.png', '.gif', '.jpg', '.jpeg', '.doc', '.docx', '.xls', '.xlsx', '.ppt', '.pptx', '.pdf']);
...
module.exports = React.createClass({
	...
	getInitialState: function() {
		return {
			...
			info: {
				id: 0,
				ontent: "",
				entityId: this.props.location.query.typeId,
				picFileIds: "",
				remarkType: this.props.location.query.type
			},
			fileList: []
		}
	...
	componentDidMount: function(param) {
		...
		let id = self.props.params.fileId;
		if (id && id > 0) {
	...
	handleChange: function(info) {
		...
		if (info.file.status === 'done') {
			...
			var t = '', 
				head = CONFIG.APIS.crm_pc,
				uidTmp = info.file.uid,
				str = info.file.name;

			if(str.match(/\.doc|\.docx/i)){
				t = head + '/assets/img/ic-word2.png';
			}else if(str.match(/\.ppt|\.pptx/i)){
				t = head + '/assets/img/ic-ppt2.png';
			}else if(str.match(/\.xls|\.xlsx/i)){
				t = head + '/assets/img/ic-excel2.png';
			}else if(str.match(/\.pdf/i)){
				t = head + '/assets/img/ic-pdf2.png';
			}

			for(var i=0; i<fileList.length; i++){
				if(fileList[i].uid == uidTmp){
					fileList[i].thumbUrl = t||info.file.url;
				}
			}
			this.setState({ fileList });
