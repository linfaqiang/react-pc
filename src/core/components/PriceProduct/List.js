...
module.exports = React.createClass({
	...
	componentDidUpdate:function(){
		if(this.state.selectedList && this.state.selectedList.length > 0){
			Tools.imgLoadError();
		}
	},
			...
			.end(function(err,res){
				if (res.ok) {
					if (res.body.code == '200') {
						...
						var selectData = self.state.selectedList;
						var arr = [];
						if(type == 'add'){
							for(var i=0,len=newData.length;i<len;i++){
								...
								for(var k=0;k<selectData.length;k++){
									if(newData[i].id == selectData[k].id){
										f=1;
									}
								}
								if(f == 0){
	...
	addProduct:function(){
		...
		arr = this.state.selectedList;
		for(var i=0,len=thisObj.length; i<len; i++){

			if(thisObj[i].checked){
				var tempArry = [];
				tempArry.id=thisObj[i].id;
				tempArry.productTypeId=thisObj[i].productTypeId;
				tempArry.productName=thisObj[i].productName;
				tempArry.productTypeText=thisObj[i].productTypeText;
				tempArry.description=thisObj[i].description;
				tempArry.productPrice=thisObj[i].productPrice;
				//tempArry.quantity=thisObj[i].quantity;
				tempArry.amount=thisObj[i].productPrice;
				tempArry.quantity=1;
				arr.push(tempArry);
			}
		}
	...
	render:function(){
		var lists = this.state.selectedList,
			len = lists.length;
